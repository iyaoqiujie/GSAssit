...
# @Time: 2020/6/19 2:56 下午
from flask import Flask, jsonify
from flask import request
from flask import abort, make_response

from workwechat_assist.utils.WXBizMsgCrypt3 import WXBizMsgCrypt
...
@app.route('/weworkcall', methods=['get'])
def wework_call():
    ...
    print('[{0}], [{1}], [{2}], [{3}]'.format(sVerifyMsgSig, sVerifyTimeStamp, sVerifyNonce, sVerifyEchoStr))

    ret, sEchoStr = wxcpt.VerifyURL(sVerifyMsgSig, sVerifyTimeStamp, sVerifyNonce, sVerifyEchoStr)
    if ret != 0:
        print("ERR: VerifyURL ret: {0}".format(ret))
        sys.exit(1)
    ...
    sReqData = request.form.get('data')
...
@app.route('/liveAuth', methods=['POST'])
def live_auth():
    userid = request.form.get('name') or ''
    if userid == '':
        abort(400)
